...
- 1280×720 클라이언트 주변에 양옆 304px, 아래 300px 프레임을 붙입니다. 하단 폭은 양옆 패널 끝에 맞추고, Windows 기본 제목 표시줄은 어두운 전용 헤더로 대체합니다.
- Pretendard 글꼴과 챔피언 초상화를 포함하며 버튼에 hover·눌림 효과를 적용합니다. 좌우 슬롯에는 현재 픽과 후보의 승률 차이, 해당 관계에 좋은 내 라인 챔피언 1·2순위를 표시합니다.
- 하단에서 전체 조합 추천 1·2순위와 `나의 주 챔피언` 점수를 비교할 수 있습니다. `설정`에서 라인별 주 챔피언을 등록하며, 추천 상위 20개 밖의 챔피언도 점수를 따로 표시합니다.
- 제목을 끌면 수동 배치로 바뀌고 세 패널이 함께 이동합니다. "기본 배치"로 클라이언트에 다시 맞출 수 있습니다.
